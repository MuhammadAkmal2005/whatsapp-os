...
- **Integration & Unit Suites**: Fully active and verified across 101 test files covering auth, tenancy, permissions, order totals, webhook HMAC verification, Meta WhatsApp provider, logical event parsing, background job processor, WhatsApp account management, Gemini AI runtime, Knowledge Base RAG grounding (`pgvector`), knowledge ingestion end to end (splitting, validation, duplicate detection, plan limits, the queue path including dead-letter retry, supersession under a concurrent edit, and metering that survives a partial failure), tool registry (including `create_order`), human handoff orchestration, automation engine, triggers & multi-step actions, wait/delayed resumption, conversation idle scanner, in-app notification lifecycle, and master end-to-end acceptance suites.
